...
try:
    ...
    class Github:
        ...
        def get_repo(self, *args, **kwargs):
            raise ImportError(
                "La librería 'PyGithub' no está instalada. Ejecuta 'pip install PyGithub' para usar esta función."
            )
...
def read_github_file(
    repo_name: str, path: str, branch: str = "main", token: str | None = None
) -> str:
    """
    ...
    try:
        ...
        return f"Error leyendo el archivo '{path}' en '{repo_name}': {e!s}"
...
def list_github_directory(
    ...
    try:
        ...
        return [f"Error listando el directorio '{path}' en '{repo_name}': {e!s}"]
