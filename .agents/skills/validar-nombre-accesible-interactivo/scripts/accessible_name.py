...
def check_accessible_name(html_str):
    ...
    elements = re.finditer(r'<(a|button)([^>]*)>(.*?)</\1>', html_str, re.IGNORECASE | re.DOTALL)
    ...
    for match in elements:
        ...
        # Chequear aria-labelledby
        elif re.search(r'aria-labelledby\s*=\s*["\']([^"\']+)["\']', attrs, re.IGNORECASE):
            has_name = True
        else:
            ...
            if text_content:
                ...
                alt_match = re.search(r'<img[^>]+alt\s*=\s*["\']([^"\']+)["\']', inner_html, re.IGNORECASE)
        ...
        results.append({
            "selector": selector,
            "tipo": tag_name,
            "tiene_nombre_accesible": has_name
        })
